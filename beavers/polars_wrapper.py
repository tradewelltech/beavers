...
class PolarsDagWrapper:
    ...
    def source_table(
        ...
        return self._dag.source_stream(empty=pl.DataFrame(schema=schema), name=name)
    ...
    def table_stream(
        ...
        return self._dag.stream(function, empty=pl.DataFrame(schema=schema))
    ...
    def last_by_keys(
        ...
        return self._dag.state(
            _LastByKey(tuple(keys), pl.DataFrame(schema=schema))
        ).map(stream)
    ...
    def get_series(self, stream: Node[pl.DataFrame], name: str) -> Node[pl.Series]:
        empty = pl.DataFrame(schema=_get_stream_schema(stream))[name]
